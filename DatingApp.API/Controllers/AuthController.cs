...
namespace DatingApp.API.Controllers
{
    ...
    public class AuthController : ControllerBase
    {
        ...
        {
            var userFromRepom = await _repo.Login(userLoginDTO.Username,userLoginDTO.Password);

            if(userFromRepom == null)
                return Unauthorized();

            var claims = new[]
            {
                new Claim(ClaimTypes.NameIdentifier, userFromRepom.Id.ToString()),
                new Claim(ClaimTypes.Name, userFromRepom.UserName)
            };
